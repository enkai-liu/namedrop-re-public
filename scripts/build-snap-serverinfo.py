...
def main() -> int:
    ...
    ap.add_argument("--java", default=os.path.join(
                        REPO, "android", "hce-probe", "app", "src", "main", "java",
                        "com", "namedrop", "hceprobe", "SnapBlobs.java"),
                    help="generated Java blobs for the Pixel HCE applet. Emitted from the SAME "
                         "identity as the C header, because the Pixel answers the NFC while Linux "
...
//
// com.apple.boop.SNAP blobs for the Pixel HCE applet (gap #2), emitted from the SAME
...
// device answering the QUIC -- the Pixel can be the card while Linux is the receiver.
